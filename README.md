...
The following three steps are done for each library: (1) SAM files are first converted to a tabular format using the ```utils/parse_bwa_sam.pl``` script, (2) read sides are paired using the ```utils/pair_reads.pl``` script, and (3) paired reads are transformed into POP files using the construct command (see the syntax of the construct command [here](docs/syntax.md)). POP files are an efficient representation of mapped reads that allows quick queries. These files are internally compressed by PolyPanner using the gzip format.
